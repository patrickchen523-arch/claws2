...
function extractGameName(content) {
  const tableMatch = content.match(/\|?\s*来源游戏\s*\|?\s*[🔗\*]*\s*([^\|\n*]+?)\s*\*/);
  if (tableMatch) {
    const name = tableMatch[1].replace(/[*🔗]/g, '').trim();
    if (name) return name;
  }
  const tableMatch2 = content.match(/\|?\s*来源游戏\s*\|?\s*🔗\s*([^\|\n]+)/);
  if (tableMatch2) {
    const name = tableMatch2[1].replace(/[*🔗]/g, '').trim();
    if (name) return name;
  }
...
for (const dir of dirs) {
  ...
  for (const file of files) {
    ...
    const population = getPopulation(gameName);
...
console.log(`添加人数规模：${added} 个，跳过 ${skipped} 个`);
